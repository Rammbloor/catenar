...
func TestBootstrapReturnsContractAndLayoutMetadata(t *testing.T) {
	...
	if response.Data.App.Name != "Catenar" {
		t.Fatalf("expected app name Catenar, got %q", response.Data.App.Name)
	}

	if response.Data.App.Version != "0.0.0" {
		t.Fatalf("expected app version 0.0.0, got %q", response.Data.App.Version)
	}
