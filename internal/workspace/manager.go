...
const (
	...
	savedRequestModeSingle = "single"
	savedRequestModeStatic = "static-sequence"
)
...
type ManagerOptions struct {
	...
	StatePath    string
	DeferRestore bool
	AccessGrant  WorkspaceAccessGrant
}
...
type manifestFile struct {
	...
	Environments  map[string]environmentFile `yaml:"environments,omitempty"`
...
type settingsFile struct {
	...
	CustomSecretKeys []string            `yaml:"customSecretKeys,omitempty"`
...
func NewManager(options ManagerOptions) *Manager {
	...
	if accessGrant == nil {
		accessGrant = defaultWorkspaceAccessGrant()
	}
	manager := &Manager{now: now, statePath: strings.TrimSpace(options.StatePath), accessGrant: accessGrant}
	if !options.DeferRestore {
		manager.RestoreLastWorkspace()
	}
	return manager
}
...
func (m *Manager) Create(_ context.Context, input contracts.WorkspaceCreateInput) contracts.WorkspaceResponse {
	...
	manifest := manifestFile{
		...
		Environments: environmentsToFile(input.Environments),
		Settings:     settingsToFile(input.Settings),
	}
	...
	m.setActive(active)
	m.persistLastWorkspace(active.manifestPath)
...
func (m *Manager) Open(_ context.Context, path string) contracts.WorkspaceResponse {
	...
	if issue := firstBlockingSavedRequestLoadIssue(issues); issue != nil {
		return workspaceFailure(*issue)
	}
	...
	return workspaceSuccessWithIssues(active.snapshot(), issues)
}

func (m *Manager) Close(_ context.Context) contracts.WorkspaceCloseResponse {
	m.setActive(nil)
	m.persistLastWorkspace("")
	return contracts.WorkspaceCloseResponse{Ok: true}
}

// Delete removes the active workspace manifest and its managed saved-request
// documents. It deliberately preserves every other file in the workspace
// folder, so opening a workspace from an existing project cannot delete that
// project by accident.
func (m *Manager) Delete(_ context.Context) contracts.WorkspaceCloseResponse {
	active := m.activeClone()
	if active == nil {
		return contracts.WorkspaceCloseResponse{Ok: false, Error: errorEnvelopeFromIssue(noActiveWorkspaceIssue())}
	}

	managedRequestsPath := filepath.Join(active.root, "requests")
	if !isWorkspaceManagedPath(active.root, active.manifestPath) || !isWorkspaceManagedPath(active.root, managedRequestsPath) {
		return contracts.WorkspaceCloseResponse{Ok: false, Error: errorEnvelopeFromIssue(workspaceIssue{
			field:    "path",
			code:     "workspace.delete_unsafe_path",
			category: contracts.ErrorCategoryWorkspace,
			message:  "The workspace files could not be deleted safely.",
			path:     active.root,
		})}
	}

	if err := os.Remove(active.manifestPath); err != nil && !errors.Is(err, os.ErrNotExist) {
		return workspaceDeleteFailure(active.manifestPath, err)
	}
	if err := os.RemoveAll(managedRequestsPath); err != nil {
		return workspaceDeleteFailure(managedRequestsPath, err)
	}
	for _, backupPath := range active.backupPaths {
		if !isWorkspaceManagedPath(active.root, backupPath) {
			continue
		}
		if err := os.Remove(backupPath); err != nil && !errors.Is(err, os.ErrNotExist) {
			return workspaceDeleteFailure(backupPath, err)
		}
	}
...
// Active returns a stable snapshot and filesystem location for the active
// workspace. Integrations use this read-only view instead of reaching into the
// manager's manifest representation.
func (m *Manager) Active() (contracts.WorkspaceSnapshot, bool) {
	active := m.activeClone()
	if active == nil {
...
func (m *Manager) Save(_ context.Context, input contracts.WorkspaceSaveInput) contracts.WorkspaceResponse {
	...
	if input.Endpoints != nil {
		...
		active.savedRequests = savedRequestsForEndpoints(active.savedRequests, endpointIDsFromFile(next.Endpoints))
		next.SavedRequests = savedRequestPaths(active.savedRequests)
	}
	...
	if input.Environments != nil {
		next.Environments = environmentsToFile(input.Environments)
	}
	if input.Settings != nil {
		next.Settings = settingsToFile(input.Settings)
	}
...
func (m *Manager) RequestSave(_ context.Context, input contracts.RequestSaveInput) contracts.RequestSaveResponse {
	...
	for _, existing := range active.savedRequests {
		if existing.ID == request.ID || existing.path == request.path {
			continue
		}
		if existing.EndpointRef == request.EndpointRef &&
			strings.EqualFold(strings.TrimSpace(existing.Name), strings.TrimSpace(request.Name)) {
			return requestFailure(workspaceIssue{
				field:    "name",
				code:     "validation.saved_request_name_duplicate",
				category: contracts.ErrorCategoryValidation,
				message:  "A saved request with the same name already exists for this endpoint.",
			})
		}
	}
...
func (m *Manager) RequestGet(_ context.Context, input contracts.RequestGetInput) contracts.RequestGetResponse {
	active := m.activeClone()
	if active == nil {
		return contracts.RequestGetResponse{
			Ok:    false,
			Error: errorEnvelopeFromIssue(noActiveWorkspaceIssue()),
		}
	}
	...
	path := filepath.ToSlash(filepath.Clean(strings.TrimSpace(input.Path)))
	if strings.TrimSpace(input.Path) == "" {
		path = ""
	}

	if id == "" && path == "" {
		return requestGetFailure(workspaceIssue{
			field:    "id",
			code:     "validation.saved_request_ref_required",
			category: contracts.ErrorCategoryValidation,
			message:  "Saved request id or path is required.",
		})
	}

	for _, request := range active.savedRequests {
		if (id != "" && request.ID == id) || (path != "" && request.path == path) {
			snapshot := active.snapshot()
			return contracts.RequestGetResponse{
				Ok: true,
				Data: &contracts.RequestGetResult{
					Workspace:    snapshot,
					SavedRequest: request.toContract(),
				},
			}
		}
	}
	...
	return requestGetFailure(workspaceIssue{
		field:    "id",
		code:     "workspace.saved_request_not_found",
		category: contracts.ErrorCategoryWorkspace,
		message:  "The saved request could not be found in the active workspace.",
	})
}

func (m *Manager) RequestDelete(_ context.Context, input contracts.RequestDeleteInput) contracts.RequestDeleteResponse {
	active := m.activeClone()
	if active == nil {
		return contracts.RequestDeleteResponse{
			Ok:    false,
			Error: errorEnvelopeFromIssue(noActiveWorkspaceIssue()),
		}
	}
	...
	if id == "" && path == "" {
		return requestDeleteFailure(workspaceIssue{
			field:    "id",
			code:     "validation.saved_request_ref_required",
			category: contracts.ErrorCategoryValidation,
			message:  "Saved request id or path is required.",
	...
	deleteIndex := -1
	for index, request := range active.savedRequests {
		if (id != "" && request.ID == id) || (path != "" && request.path == path) {
			deleteIndex = index
			break
		}
	...
	deleted := active.savedRequests[deleteIndex]
	absolutePath, issue := workspaceRelativePath(active.root, deleted.path, "savedRequests")
	if issue != nil {
		return requestDeleteFailure(*issue)
	}
	if err := os.Remove(absolutePath); err != nil && !errors.Is(err, os.ErrNotExist) {
		return requestDeleteFailure(workspaceIssue{
			field:    "savedRequests",
			code:     "workspace.saved_request_delete_failed",
			category: contracts.ErrorCategoryWorkspace,
			message:  "The saved request file could not be deleted.",
			path:     absolutePath,
		})
	}

	nextRequests := make([]savedRequestFile, 0, len(active.savedRequests)-1)
	nextRequests = append(nextRequests, active.savedRequests[:deleteIndex]...)
	nextRequests = append(nextRequests, active.savedRequests[deleteIndex+1:]...)

	nextManifest := active.manifest
	nextManifest.SavedRequests = removeString(nextManifest.SavedRequests, deleted.path)
	if err := writeManifestFile(active.manifestPath, nextManifest); err != nil {
		return requestDeleteFailure(workspaceIssue{
			field:    "savedRequests",
			code:     "workspace.manifest_write_failed",
			category: contracts.ErrorCategoryWorkspace,
			message:  "The workspace manifest could not be updated after deleting the saved request.",
			path:     active.manifestPath,
		})
	}

	active.manifest = nextManifest
	active.savedRequests = nextRequests
	m.setActive(active)

	return contracts.RequestDeleteResponse{
		Ok: true,
		Data: &contracts.RequestDeleteResult{
			Workspace: active.snapshot(),
...
func (m *Manager) PrepareEndpointTest(_ context.Context, input contracts.EndpointTestInput) (endpoint.WorkspaceContext, contracts.EndpointPreset, error) {
	...
	return endpoint.WorkspaceContext{
		...
		Environments:   environmentValuesFromFile(m.active.manifest.Environments),
		Redaction:      metadataRedactionPolicyFromSettingsFile(m.active.manifest.Settings),
	}, normalized, nil
}
...
type persistedRuntimeState struct {
	...
	LastWorkspaceBookmark string `json:"lastWorkspaceBookmark,omitempty"`
}

// RestoreLastWorkspace restores the last workspace after the host runtime is
// ready. A moved, deleted, or malformed manifest must never make startup fail.
// Desktop hosts can defer this call until their file-access runtime is live.
func (m *Manager) RestoreLastWorkspace() contracts.WorkspaceResponse {
	if m.statePath == "" {
		return contracts.WorkspaceResponse{Ok: true}
	}
	...
	if err != nil {
		...
		return workspaceFailure(workspaceIssue{
			field:    "runtimeState",
			code:     "workspace.runtime_state_unreadable",
			category: contracts.ErrorCategoryWorkspace,
			message:  "The saved workspace state could not be read.",
			path:     m.statePath,
		})
	}
	var state persistedRuntimeState
	if json.Unmarshal(content, &state) != nil || strings.TrimSpace(state.LastWorkspacePath) == "" {
		return contracts.WorkspaceResponse{Ok: true}
	}

	path := state.LastWorkspacePath
	bookmark := strings.TrimSpace(state.LastWorkspaceBookmark)
	if bookmark == "" {
		// Upgrade pre-bookmark runtime state on the first launch that supports
		// persistent access. This is best effort: macOS still refuses a path the
		// user has never authorised through a native open dialog.
		bookmark, _ = m.accessGrant.Remember(path)
	}
	if bookmark != "" {
		if resolvedPath, resolveErr := m.accessGrant.Restore(bookmark); resolveErr == nil && strings.TrimSpace(resolvedPath) != "" {
			path = resolvedPath
		}
	}
	return m.Open(context.Background(), path)
}

func (m *Manager) persistLastWorkspace(manifestPath string) {
	if m.statePath == "" {
		return
	}
	...
	bookmark := ""
	if manifestPath != "" {
		bookmark, _ = m.accessGrant.Remember(manifestPath)
	}
	payload, err := json.Marshal(persistedRuntimeState{
		LastWorkspacePath:     manifestPath,
		LastWorkspaceBookmark: bookmark,
	})
	if err != nil {
		return
	}
	...
	_ = os.Rename(temporary, m.statePath)
}
...
func (m *Manager) activeClone() *activeWorkspace {
	...
	cloned.manifest.Environments = cloneEnvironmentFiles(m.active.manifest.Environments)
	cloned.manifest.SavedRequests = append([]string(nil), m.active.manifest.SavedRequests...)
	cloned.manifest.Settings = cloneSettingsFile(m.active.manifest.Settings)
...
func (a *activeWorkspace) snapshot() contracts.WorkspaceSnapshot {
	return contracts.WorkspaceSnapshot{
		...
		Environments:  environmentsFromFile(a.manifest.Environments),
		Settings:      settingsFromFile(a.manifest.Settings),
		SavedRequests: savedRequestSummaries(a.savedRequests),
		BackupPaths:   append([]string(nil), a.backupPaths...),
	}
}

func isWorkspaceManagedPath(root string, candidate string) bool {
	...
	for index, ref := range refs {
		...
		if err != nil {
			issues = append(issues, workspaceIssue{
				...
				code:     "workspace.saved_request_unreadable",
				category: contracts.ErrorCategoryWorkspace,
		...
		if err := yaml.Unmarshal(content, &request); err != nil {
			issues = append(issues, workspaceIssue{
				field:    fmt.Sprintf("savedRequests[%d]", index),
				code:     "workspace.saved_request_invalid",
				category: contracts.ErrorCategoryWorkspace,
...
func firstBlockingSavedRequestLoadIssue(issues []workspaceIssue) *workspaceIssue {
	for index := range issues {
		if issues[index].code == "workspace.saved_request_ref_invalid" {
			return &issues[index]
		}
	}
...
func validateManifest(root string, manifest manifestFile, savedRequests []savedRequestFile) []workspaceIssue {
	var issues []workspaceIssue
	redaction := metadataRedactionPolicyFromSettingsFile(manifest.Settings)
	if manifest.Version != CurrentManifestVersion {
		issues = append(issues, workspaceIssue{
			field:    "version",
			code:     "workspace.version_unsupported",
	...
	for index, endpointFile := range manifest.Endpoints {
		...
		issues = append(issues, validateMetadataSecretRefs(fieldPrefix+".metadataDefaults", endpointPreset.MetadataDefaults, redaction)...)
	}

	issues = append(issues, validateSettingsFile(manifest.Settings)...)

	environmentIDs := make(map[string]struct{}, len(manifest.Environments))
	for name, environment := range manifest.Environments {
		trimmedName := strings.TrimSpace(name)
		fieldPrefix := fmt.Sprintf("environments.%s", name)
		if trimmedName == "" {
			issues = append(issues, workspaceIssue{
				field:    "environments",
				code:     "validation.workspace_environment_name_required",
				category: contracts.ErrorCategoryValidation,
				message:  "Workspace environment names must not be empty.",
			})
			continue
		}
		if trimmedName != name {
			issues = append(issues, workspaceIssue{
				field:    fieldPrefix,
				code:     "validation.workspace_environment_name_invalid",
				category: contracts.ErrorCategoryValidation,
				message:  "Workspace environment names must not have leading or trailing whitespace.",
			})
		}
		environmentIDs[trimmedName] = struct{}{}
		for key, value := range environment.Values {
			if strings.TrimSpace(key) == "" {
				issues = append(issues, workspaceIssue{
					field:    fieldPrefix + ".values",
					code:     "validation.workspace_environment_key_required",
					category: contracts.ErrorCategoryValidation,
					message:  "Workspace environment keys must not be empty.",
				})
			}
			if strings.TrimSpace(value) == "" {
				continue
			}
			if strings.HasPrefix(strings.TrimSpace(value), "secret-ref:") {
				if _, err := endpoint.ParseSecretRef(strings.TrimSpace(value)); err != nil {
					issues = append(issues, secretRefIssueFromError(fieldPrefix+".values."+key, err))
				}
			}
		}
	}
	...
	for index, request := range savedRequests {
		issues = append(issues, validateSavedRequest(index, request, endpointIDs, environmentIDs, redaction)...)
	}
...
func validateSettingsFile(settings *settingsFile) []workspaceIssue {
	if settings == nil {
		return nil
	}

	var issues []workspaceIssue
	seenSecretKeys := make(map[string]struct{}, len(settings.CustomSecretKeys))
	for index, key := range settings.CustomSecretKeys {
		trimmedKey := strings.TrimSpace(key)
		field := fmt.Sprintf("settings.customSecretKeys[%d]", index)
		if trimmedKey == "" {
			issues = append(issues, workspaceIssue{
				field:    field,
				code:     "validation.workspace_custom_secret_key_required",
				category: contracts.ErrorCategoryValidation,
				message:  "Workspace custom secret keys must not be empty.",
			})
			continue
		}
		if trimmedKey != key {
			issues = append(issues, workspaceIssue{
				field:    field,
				code:     "validation.workspace_custom_secret_key_invalid",
				category: contracts.ErrorCategoryValidation,
				message:  "Workspace custom secret keys must not have leading or trailing whitespace.",
			})
		}

		normalizedKey := strings.ToLower(trimmedKey)
		if _, exists := seenSecretKeys[normalizedKey]; exists {
			issues = append(issues, workspaceIssue{
				field:    field,
				code:     "validation.workspace_custom_secret_key_duplicate",
				category: contracts.ErrorCategoryValidation,
				message:  "Workspace custom secret keys must be unique case-insensitively.",
			})
			continue
		}
		seenSecretKeys[normalizedKey] = struct{}{}
	}

	return issues
}

func validateSavedRequest(index int, request savedRequestFile, endpointIDs, environmentIDs map[string]struct{}, redaction endpoint.MetadataRedactionPolicy) []workspaceIssue {
	...
	if strings.TrimSpace(request.EnvironmentRef) != "" {
		if _, exists := environmentIDs[strings.TrimSpace(request.EnvironmentRef)]; !exists {
			issues = append(issues, workspaceIssue{
				field:    fieldPrefix + ".environmentRef",
				code:     "validation.saved_request_environment_ref_invalid",
				category: contracts.ErrorCategoryValidation,
				message:  "Saved request environmentRef must point to an environment in the workspace manifest.",
				path:     request.path,
			})
		}
	}
	...
	} else if request.RequestSpec.Mode != savedRequestModeSingle && request.RequestSpec.Mode != savedRequestModeStatic {
		issues = append(issues, workspaceIssue{
			field:    fieldPrefix + ".requestSpec.mode",
			code:     "validation.saved_request_spec_mode_invalid",
			category: contracts.ErrorCategoryValidation,
			message:  "Saved request requestSpec.mode must be single or static-sequence.",
			path:     request.path,
		})
	} else if request.RPCType == contracts.RPCTypeUnary && request.RequestSpec.Mode != savedRequestModeSingle {
		issues = append(issues, workspaceIssue{
			field:    fieldPrefix + ".requestSpec.mode",
			code:     "validation.saved_request_spec_mode_invalid",
			category: contracts.ErrorCategoryValidation,
			message:  "Unary saved requests must use requestSpec.mode single.",
			path:     request.path,
		})
	}
	issues = append(issues, validateMetadataSecretRefs(fieldPrefix+".metadataTemplate", request.MetadataTemplate, redaction)...)

	return issues
}

func validateMetadataSecretRefs(fieldPrefix string, metadata map[string]string, redaction endpoint.MetadataRedactionPolicy) []workspaceIssue {
	var issues []workspaceIssue
	for key, value := range metadata {
		...
		if endpoint.MetadataKeyIsSensitive(key, redaction) {
			issues = append(issues, workspaceIssue{
				field:    field,
...
func requestFileFromInput(input contracts.RequestSaveInput) (savedRequestFile, *workspaceIssue) {
	...
	name := strings.TrimSpace(input.Name)
	if name == "" {
		name = savedRequestDisplayNameWithFallback("", input.Method)
	}
...
func environmentsToFile(environments map[string]contracts.WorkspaceEnvironment) map[string]environmentFile {
	if len(environments) == 0 {
		return nil
	}

	result := make(map[string]environmentFile, len(environments))
	for name, environment := range environments {
		trimmedName := strings.TrimSpace(name)
		if trimmedName == "" {
			result[name] = environmentFile{Values: copyStringMap(environment.Values)}
			continue
		}
		result[trimmedName] = environmentFile{Values: copyStringMap(environment.Values)}
	}
	return result
}

func environmentsFromFile(environments map[string]environmentFile) map[string]contracts.WorkspaceEnvironment {
	if len(environments) == 0 {
		return nil
	}

	result := make(map[string]contracts.WorkspaceEnvironment, len(environments))
	for name, environment := range environments {
		result[name] = contracts.WorkspaceEnvironment{Values: copyStringMap(environment.Values)}
	}
	return result
}

func environmentValuesFromFile(environments map[string]environmentFile) map[string]map[string]string {
	if len(environments) == 0 {
		return nil
	}

	result := make(map[string]map[string]string, len(environments))
	for name, environment := range environments {
		result[name] = copyStringMap(environment.Values)
	}
	return result
}

func cloneEnvironmentFiles(environments map[string]environmentFile) map[string]environmentFile {
	if len(environments) == 0 {
		return nil
	}

	cloned := make(map[string]environmentFile, len(environments))
	for name, environment := range environments {
		cloned[name] = environmentFile{Values: copyStringMap(environment.Values)}
	}
	return cloned
}

func settingsToFile(settings *contracts.WorkspaceSettings) *settingsFile {
	if settings == nil {
		return nil
	}

	result := &settingsFile{
		RedactDefaults:   settings.RedactDefaults,
		CustomSecretKeys: cleanStringList(settings.CustomSecretKeys),
	}
	if settings.EventRetention != nil {
		result.EventRetention = &eventRetentionFile{
			MaxEventsPerCall: settings.EventRetention.MaxEventsPerCall,
			MaxBytesPerCall:  settings.EventRetention.MaxBytesPerCall,
		}
	}
	return result
}

func settingsFromFile(settings *settingsFile) *contracts.WorkspaceSettings {
	if settings == nil {
		return nil
	}

	result := &contracts.WorkspaceSettings{
		RedactDefaults:   settings.RedactDefaults,
		CustomSecretKeys: cleanStringList(settings.CustomSecretKeys),
	}
	if settings.EventRetention != nil {
		result.EventRetention = &contracts.WorkspaceEventRetentionSettings{
			MaxEventsPerCall: settings.EventRetention.MaxEventsPerCall,
			MaxBytesPerCall:  settings.EventRetention.MaxBytesPerCall,
		}
	}
...
func cloneSettingsFile(settings *settingsFile) *settingsFile {
	if settings == nil {
		return nil
	}
	...
	cloned := &settingsFile{
		RedactDefaults:   settings.RedactDefaults,
		CustomSecretKeys: append([]string(nil), settings.CustomSecretKeys...),
	}
	if settings.EventRetention != nil {
		cloned.EventRetention = &eventRetentionFile{
			MaxEventsPerCall: settings.EventRetention.MaxEventsPerCall,
			MaxBytesPerCall:  settings.EventRetention.MaxBytesPerCall,
		}
	}
...
func metadataRedactionPolicyFromSettingsFile(settings *settingsFile) endpoint.MetadataRedactionPolicy {
	if settings == nil {
		return endpoint.DefaultMetadataRedactionPolicy()
	}

	return endpoint.NewMetadataRedactionPolicy(settings.RedactDefaults, settings.CustomSecretKeys)
}
...
func savedRequestPaths(requests []savedRequestFile) []string {
...
func endpointIDsFromFile(endpoints []endpointFile) map[string]struct{} {
	result := make(map[string]struct{}, len(endpoints))
	for _, endpointFile := range endpoints {
		if strings.TrimSpace(endpointFile.ID) != "" {
			result[strings.TrimSpace(endpointFile.ID)] = struct{}{}
		}
	}
	return result
}

func savedRequestsForEndpoints(requests []savedRequestFile, endpointIDs map[string]struct{}) []savedRequestFile {
	if len(requests) == 0 {
		return nil
	}

	filtered := make([]savedRequestFile, 0, len(requests))
	for _, request := range requests {
		if _, exists := endpointIDs[request.EndpointRef]; exists {
...
func (r savedRequestFile) summary() contracts.WorkspaceSavedRequestSummary {
	return contracts.WorkspaceSavedRequestSummary{
		ID:             r.ID,
		Name:           savedRequestDisplayNameWithFallback(r.Name, r.Method),
		Path:           r.path,
		Method:         r.Method,
		RPCType:        r.RPCType,
...
func (r savedRequestFile) toContract() contracts.WorkspaceSavedRequest {
	return contracts.WorkspaceSavedRequest{
		ID:               r.ID,
		Name:             savedRequestDisplayNameWithFallback(r.Name, r.Method),
		Path:             r.path,
		Method:           r.Method,
		...
		EndpointRef:      r.EndpointRef,
		EnvironmentRef:   r.EnvironmentRef,
		MetadataTemplate: copyStringMap(r.MetadataTemplate),
		CallOptions: contracts.CallOptions{
			RequestTimeoutMs:    r.CallOptions.RequestTimeoutMs,
			StreamIdleTimeoutMs: r.CallOptions.StreamIdleTimeoutMs,
		},
		RequestSpec: contracts.SavedRequestSpec{
			Mode:     r.RequestSpec.Mode,
			Body:     r.RequestSpec.Body,
			Messages: append([]contracts.StreamMessage(nil), r.RequestSpec.Messages...),
		},
	}
}

func savedRequestDisplayNameWithFallback(name, method string) string {
	if trimmed := strings.TrimSpace(name); trimmed != "" {
		return trimmed
	}
	parts := strings.FieldsFunc(strings.TrimSpace(method), func(r rune) bool {
		return r == '.' || r == '/'
	})
	if len(parts) > 0 {
...
func workspaceSuccess(snapshot contracts.WorkspaceSnapshot) contracts.WorkspaceResponse {
	return workspaceSuccessWithIssues(snapshot, nil)
}

func workspaceSuccessWithIssues(snapshot contracts.WorkspaceSnapshot, issues []workspaceIssue) contracts.WorkspaceResponse {
...
func workspaceDeleteFailure(path string, cause error) contracts.WorkspaceCloseResponse {
	envelope := errorEnvelopeFromIssue(workspaceIssue{
		field:    "path",
		code:     "workspace.delete_failed",
		category: contracts.ErrorCategoryWorkspace,
		message:  "The workspace files could not be deleted.",
		path:     path,
	})
	envelope.Details = map[string]string{"cause": cause.Error()}
	return contracts.WorkspaceCloseResponse{Ok: false, Error: envelope}
}
...
func requestGetFailure(issue workspaceIssue) contracts.RequestGetResponse {
	return contracts.RequestGetResponse{
		Ok:    false,
		Error: errorEnvelopeFromIssue(issue),
	}
...
func requestDeleteFailure(issue workspaceIssue) contracts.RequestDeleteResponse {
	return contracts.RequestDeleteResponse{
		Ok:    false,
		Error: errorEnvelopeFromIssue(issue),
	}
