...
const (
	MaterialKindTLSCA         MaterialKind = "tls_ca"
	MaterialKindTLSClientCert MaterialKind = "tls_client_cert"
	MaterialKindTLSClientKey  MaterialKind = "tls_client_key"
	MaterialKindMetadata      MaterialKind = "metadata"
)

type MaterialRegisterFileInput struct {
...
type MaterialRegisterFileResult struct {
	Ref    string             `json:"ref"`
	Record MaterialFileRecord `json:"record"`
}

type MaterialRegisterFileResponse struct {
	Ok    bool                        `json:"ok"`
	Data  *MaterialRegisterFileResult `json:"data,omitempty"`
	Error *ErrorEnvelope              `json:"error,omitempty"`
}
...
// CatalogField describes one field from the request or response protobuf
// message. Fields expands one nested message level so the client can show the
// concrete shape of repeated response objects without serialising an
// unbounded descriptor graph.
...
type DiagnosticsExportResponse struct {
	Ok    bool                     `json:"ok"`
	Data  *DiagnosticsExportResult `json:"data,omitempty"`
	Error *ErrorEnvelope           `json:"error,omitempty"`
}
