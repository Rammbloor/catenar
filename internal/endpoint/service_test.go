	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			response := service.CallInvokeUnary(context.Background(), contracts.CallInvokeUnaryInput{
				...
				Method:        "catenar.demo.v1.ReflectionDemo.Ping",
				Body:          map[string]any{},
			})
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			response := service.CallInvokeUnary(context.Background(), contracts.CallInvokeUnaryInput{
				...
				Method:        "catenar.demo.v1.ReflectionDemo.Missing",
				Body:          map[string]any{},
			})
			...
			if response.Error.Details["method"] != "catenar.demo.v1.ReflectionDemo.Missing" {
...
func TestHistoryListFiltersCallsByEndpointID(t *testing.T) {
	t.Parallel()
	...
	now := time.Now().UTC()
	summaries := []contracts.HistoryCallSummary{
		{
			CallID:       "endpoint-a-new",
		...
		},
		{
			...
			FinishedAt:   now.Add(-2 * time.Minute).Format(time.RFC3339Nano),
			RequestCount: 1,
		},
	}

	for _, summary := range summaries {
		if err := service.historyStore.SaveCallSummary(context.Background(), summary); err != nil {
			t.Fatalf("save call summary %s: %v", summary.CallID, err)
		}
	}

	response := service.HistoryList(context.Background(), contracts.HistoryListInput{
		EndpointID: "endpoint-a",
		Limit:      10,
	})
	if !response.Ok || response.Data == nil {
		t.Fatalf("expected filtered history list, got %+v", response.Error)
	}
	if len(response.Data.Calls) != 2 {
		t.Fatalf("expected two endpoint-a calls, got %+v", response.Data.Calls)
	}
	if response.Data.Calls[0].CallID != "endpoint-a-new" || response.Data.Calls[1].CallID != "endpoint-a-old" {
		t.Fatalf("expected endpoint-a calls ordered newest first, got %+v", response.Data.Calls)
	}
	for _, call := range response.Data.Calls {
		if call.EndpointID != "endpoint-a" {
			t.Fatalf("expected only endpoint-a calls, got %+v", response.Data.Calls)
		}
	}

	missingResponse := service.HistoryList(context.Background(), contracts.HistoryListInput{
		EndpointID: "endpoint-missing",
		Limit:      10,
	})
	if !missingResponse.Ok || missingResponse.Data == nil {
		t.Fatalf("expected empty filtered history list, got %+v", missingResponse.Error)
	}
	if len(missingResponse.Data.Calls) != 0 {
		t.Fatalf("expected no calls for missing endpoint, got %+v", missingResponse.Data.Calls)
	}
}

func TestHistoryListPersistsAndFiltersEnvironmentRef(t *testing.T) {
	t.Parallel()
	...
	if len(response.Data.Calls) != 1 || response.Data.Calls[0].CallID != "local-call" {
		t.Fatalf("expected only the local workspace-a call, got %+v", response.Data.Calls)
	}
	if response.Data.Calls[0].EnvironmentRef != "local" {
		t.Fatalf("expected environment ref to round-trip, got %+v", response.Data.Calls[0])
	}
}
