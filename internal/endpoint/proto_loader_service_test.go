...
func TestCatalogLoadFromProtoSourcesSupportsMultiDirectoryImportsAndBuildsRequestTemplates(t *testing.T) {
	...
	selectedMethod := findCatalogMethod(t, response.Data.Services, "catenar.demo.v1.ReflectionDemo.Ping")
...
func TestCatalogLoadFromProtoSourcesPrefersSelectedSourceRootsOverImportPaths(t *testing.T) {
	...
	template, ok := response.Data.RequestTemplates["catenar.demo.v1.ReflectionDemo.UseSource"]
	if !ok {
...
message ProtoImportMarker {
  catenar.demo.shared.v1.ImportedMarker marker = 1;
}
...
`,
	...
	writeProtoFile(
		...
		`syntax = "proto3";
package catenar.demo.shared.v1;
...
service ReflectionDemo {
  rpc UseSource(catenar.demo.shared.v1.SourceRequest) returns (google.protobuf.Empty);
}
`,
	)
