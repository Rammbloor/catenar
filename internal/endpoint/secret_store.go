	...
	if record.Kind != "" && record.Kind != expectedKind {
		return ResolvedMaterial{}, &classifiedError{
			Envelope: contracts.ErrorEnvelope{
				...
				Message:  "The registered file material kind does not match the field that references it.",
				Details: map[string]string{
			...
			NextStep: "Re-register the file under the correct material field and try again.",
		}
	}
...
func newJSONMaterialIndex(path string) *jsonMaterialIndex {
