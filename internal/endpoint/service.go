...
type WorkspaceContext struct {
	...
	Redaction      MetadataRedactionPolicy
}
...
const (
	...
	SecretUsageMetadata      SecretUsageIntent = "metadata"
)
...
func NewService(deps ServiceDependencies) *Service {
	...
	materialIndex := newJSONMaterialIndex(materialIndexPath(deps.AppDataDir))

	store := deps.SecretStore
	if store == nil {
		store = NewFileSecretStore(materialIndex)
	}
...
func (s *Service) CallInvokeUnary(ctx context.Context, input contracts.CallInvokeUnaryInput) contracts.CallInvokeUnaryResponse {
	...
	startedAt := s.now().UTC()
	mergedMetadata, metadataDiagnostic := s.resolveInvokeMetadata(ctx, cachedCatalog.scope, cachedCatalog.endpoint, input.EnvironmentRef, input.Metadata)
	if metadataDiagnostic != nil {
		s.emitDiagnostic("unary-invoke", startedAt, metadataDiagnostic)
		return contracts.CallInvokeUnaryResponse{
			Ok:    false,
			Error: errorEnvelopeFromDiagnostic(metadataDiagnostic, "validation.metadata_template_invalid", "The unary request metadata is invalid."),
		}
	}

	runtimeCfg, prepDiagnostic := s.resolveRuntimeConfig(ctx, cachedCatalog.scope, cachedCatalog.endpoint)
	if prepDiagnostic != nil {
		s.emitDiagnostic("unary-invoke", startedAt, prepDiagnostic)
		return contracts.CallInvokeUnaryResponse{
			Ok:    false,
	...
	artifacts, artifactErr := s.eventLog.WriteUnaryCall(ctx, UnaryEventLogRecord{
		...
		RedactionPolicy: cachedCatalog.scope.Redaction,
	})
	if artifactErr != nil {
		return contracts.CallInvokeUnaryResponse{
			Ok:    false,
...
func (s *Service) emitDiagnostic(source string, ts time.Time, diagnostic *endpointDiagnostic) *contracts.DiagnosticsUpdateEvent {
	...
	s.rememberDiagnostic(*event)

	return event
}

func (s *Service) rememberDiagnostic(event contracts.DiagnosticsUpdateEvent) {
	...
	if len(s.diagnostics) > maxDiagnostics {
		s.diagnostics = append([]contracts.DiagnosticsUpdateEvent(nil), s.diagnostics[len(s.diagnostics)-maxDiagnostics:]...)
	}
}

func (s *Service) diagnosticsSnapshot() []contracts.DiagnosticsUpdateEvent {
	s.diagnosticsMu.Lock()
	defer s.diagnosticsMu.Unlock()

	return append([]contracts.DiagnosticsUpdateEvent(nil), s.diagnostics...)
}
...
func appDataBaseDir(appDataDir string) string {
	...
	if err != nil {
		return filepath.Join(os.TempDir(), "catenar")
	}

	return migratedAppDataBaseDir(userConfigDir)
}

func migratedAppDataBaseDir(userConfigDir string) string {
	currentDir := filepath.Join(userConfigDir, "catenar")
	if _, err := os.Stat(currentDir); err == nil {
		return currentDir
	}

	legacyDir := filepath.Join(userConfigDir, "tether")
	...
	// Keep using the existing data when the host blocks a one-time rename.
	return legacyDir
}
...
func readStoredUnaryHistoryDetail(path string) (storedUnaryHistoryDetail, error) {
	...
	sanitized := detail
	sanitized.Headers = redactMetadataValues(detail.Headers, DefaultMetadataRedactionPolicy())
	sanitized.Trailers = redactMetadataValues(detail.Trailers, DefaultMetadataRedactionPolicy())
