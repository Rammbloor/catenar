...
func (s *Service) CallStartStream(ctx context.Context, input contracts.CallStartStreamInput) contracts.CallStartStreamResponse {
	...
	if metadataDiagnostic != nil {
		s.emitDiagnostic("server-stream", startedAt, metadataDiagnostic)
		return contracts.CallStartStreamResponse{
			Ok:    false,
			Error: errorEnvelopeFromDiagnostic(metadataDiagnostic, "validation.metadata_template_invalid", "The server-stream request metadata is invalid."),
		}
	}
...
func (s *Service) callStartClientStream(
	...
	mergedMetadata, metadataDiagnostic := s.resolveInvokeMetadata(ctx, cachedCatalog.scope, cachedCatalog.endpoint, input.EnvironmentRef, input.Metadata)
	if metadataDiagnostic != nil {
		s.emitDiagnostic("client-stream", startedAt, metadataDiagnostic)
		return contracts.CallStartStreamResponse{
			Ok:    false,
			Error: errorEnvelopeFromDiagnostic(metadataDiagnostic, "validation.metadata_template_invalid", "The client-stream request metadata is invalid."),
		}
	}
...
func (s *Service) callStartBidiStreamInteractive(
	...
	mergedMetadata, metadataDiagnostic := s.resolveInvokeMetadata(ctx, cachedCatalog.scope, cachedCatalog.endpoint, input.EnvironmentRef, input.Metadata)
	if metadataDiagnostic != nil {
		s.emitDiagnostic("bidi-stream", startedAt, metadataDiagnostic)
		return contracts.CallStartStreamResponse{
			Ok:    false,
			Error: errorEnvelopeFromDiagnostic(metadataDiagnostic, "validation.metadata_template_invalid", "The bidirectional stream request metadata is invalid."),
		}
	}
...
func (s *Service) runServerStreamSession(ctx context.Context, input streamSessionRunInput) {
	...
	artifacts, artifactErr := s.eventLog.WriteStreamCall(ctx, StreamEventLogRecord{
		...
		RedactionPolicy: input.scope.Redaction,
	})
	if artifactErr != nil {
...
func (s *Service) completeBidiStreamSession(
	...
	artifacts, artifactErr := s.eventLog.WriteStreamCall(ctx, StreamEventLogRecord{
		...
		Status:          status,
		Events:          events,
		RedactionPolicy: client.scope.Redaction,
	})
	if artifactErr != nil {
	...
	summary := contracts.HistoryCallSummary{
		...
		EnvironmentRef: client.environmentRef,
...
func (r *streamHistoryRecorder) recordCallStarted(workspaceID, endpointID string, requestBody any, metadata map[string]string, ts time.Time) recordedStreamEvent {
	return r.append(contracts.HistoryLogEvent{
		...
		GRPC: &contracts.HistoryLogGRPC{
			Method:   r.method,
			RPCType:  r.rpcType,
			Metadata: toRepeatedMetadata(redactMetadataMap(metadata, r.redaction)),
		},
		Details: map[string]string{
...
func (r *streamHistoryRecorder) recordHeaders(headers map[string][]string, ts time.Time) recordedStreamEvent {
	redactedHeaders := redactMetadataValues(headers, r.redaction)
	return r.append(contracts.HistoryLogEvent{
...
func (r *streamHistoryRecorder) recordTrailers(status contracts.StreamStatus, trailers map[string][]string, ts time.Time) recordedStreamEvent {
	redactedTrailers := redactMetadataValues(trailers, r.redaction)
	return r.append(contracts.HistoryLogEvent{
